...
var settings = {
    ...
    dest: './dist/',
    nugetPath: './nuget.exe',
    subModules: ['jquery.facebox'], // All of the git submodule names (individual releases) for the build
    version: '',
    debug: true
};
...
gulp.task('copy-minified', ['uglify', 'uglify-css', 'distribute-assets'], function () {
...
gulp.task('uglify', ['clean', 'test'], function () {
    return gulp.src(settings.src, { base: './' })
        .pipe(rename(function (path) {
            path.basename = settings.baseProject;
...
gulp.task('uglify-css', ['clean'], function () {
    return gulp.src(settings.src_css, { base: './' })
        .pipe(rename(function (path) {
            path.basename = settings.baseProject;
            path.dirname = path.basename;
        }))
